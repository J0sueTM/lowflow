...
#define LF_COLORED_LEVEL_STR_SIZE                                    \
  (LF_COLOR_PREFIX_SIZE + LF_COLOR_SUFFIX_SIZE + LF_LEVEL_STR_SIZE + \
   LF_COLOR_RESET_SIZE)
...
void lf_log(LF_Logger *logger,
  ...
  size_t time_size = strftime(head_block->data,
  ...
  size_t level_str_size =
    ((logger->is_colored) ? LF_COLORED_LEVEL_STR_SIZE : LF_LEVEL_STR_SIZE);
  const char *level_str =
    ((logger->is_colored) ? colored_level_strs[level] : level_strs[level]);
  size_t padded_level_str_size = level_str_size + 2;
  char *level_buf = lf_alloc_string(&logger->buf, padded_level_str_size);
  level_buf[0] = ' ';
  ...
  char *msg_buf = head_block->data + head_block->right_offset;
  size_t remaining_size = head_block->size - head_block->right_offset;
  size_t msg_needed_size = vsnprintf(msg_buf, remaining_size, fmt, ap);
