...
char *lf_init_string_iteration(LF_StringIteration *iter) {
  ...
  iter->cur_block = (iter->str->slice) ? iter->str->slice->head_block : iter->str->arena->head_block;
  iter->block_offset = (iter->str->slice) ? iter->str->slice->head_offset : iter->str->arena->head_block->left_offset;
...
char *lf_iterate_string(LF_StringIteration *iter) {
  ...
  LF_MemBlock *last_block = (iter->str->slice) ? iter->str->slice->tail_block : iter->str->arena->tail_block;
  size_t right_offset = (iter->str->slice) ? iter->str->slice->tail_offset : iter->str->arena->tail_block->right_offset;
...
bool lf_compare_string(LF_StringComparison *comp) {
  ...
  LF_MemBlock *cur_block = (comp->left->slice) ? comp->left->slice->head_block : comp->left->arena->head_block;
  ...
  case LF_STR_COMPARISON_STR_CHAR: {
    ...
    size_t compared = 0;
    size_t remaining = (comp->right_char_size < left_str_size) ? comp->right_char_size : left_str_size;
    while (cur_block) {
      size_t right_padding = cur_block->size - cur_block->right_offset;
      size_t used_size = cur_block->size - right_padding - cur_block->left_offset;
      ...
      int diff = memcmp(cur_block->data + cur_block->left_offset, comp->right_char + compared, comp_size);
      if (diff != 0) {
        return false;
      }
