                  ...
                  size_t arg_count, Type return_type, Type val_type)
{
    ...
    log_trace("beg alloc func_id [name='%s', argc=%lld]", name,
              arg_count);
    ...
    if (arg_count > 0) {
        ...
        id->minor_specs[0].type_by_name =
          hashdict_alloc(mod->func_arg_type_by_name_hds_arena, NULL,
                         arg_count, arg_count);
    }
    ...
    hashdict_add_entry(mod->id_by_name, name, name_size, (char *) id,
                       sizeof(ID));
...
// This is used for places where an allocated type is needed, like when
// using a type as a hashdict value.
static Type allocated_types[TYPE_COUNT] = {
...
)
{
    ...
    log_trace("beg func_id_add_arg [func_id=%p, func_name=%s, arg_name=%s, arg_type=%d]",
              func_id, func_id->name, name, type);
    ...
    HashDictEntry *arg_entry =
        hashdict_add_entry(func_id->minor_specs[0].type_by_name, name,
                           name_size,
                           (char *)&allocated_types[allocated_type_idx],
                           sizeof(Type));

    log_debug("end func_id_add_arg entry(%p) [func_id=%p, arg_name=%s, arg_type=%d]",
              arg_entry, func_id, name, type);
