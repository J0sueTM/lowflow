                          ...
                          bool should_recurse)
{
    ...
    log_debug("free data(%p) [region=%p, used=%lld]", top->data,
              bytes_freed, top->data, top->used);
    free(top->data);
...
Arena *arena_alloc(size_t region_cap)
{
    ...
    log_debug("alloc data(%p) [cap=%lld, arena=%p, mregion=%p]",
              arena->top->data, arena->region_cap, arena, arena->top);
...
void *arena_malloc(Arena *arena, size_t size)
{
    ...
    // not the ones allocating stuff, i.e. the end user of this language.
    assert(
