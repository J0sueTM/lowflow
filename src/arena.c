...
static size_t region_free(MemRegion *top,
                          bool should_recurse) {
  assert(top);
  assert(top->data);
  ...
  return bytes_freed + region_free(next_region,
                                   should_recurse);
}
...
Arena *arena_alloc(size_t region_cap) {
  ...
  arena->top->data = (char *)calloc(arena->region_cap,
                                    sizeof(char));
  if (!arena->top->data) {
    log_fatal("alloc data [cap=%lld, arena=%p, mregion=%p]", arena->region_cap,
              arena, arena->top);
    free(arena);
    free(arena->top);
    return NULL;
  }
  log_debug("alloc data(%p) [cap=%lld, arena=%p, mregion=%p]", arena->top->data,
            arena->region_cap, arena, arena->top);
  ...
  log_debug("end alloc arena(%p) [region_cap=%lld]", *arena,
            arena->region_cap);
...
void *arena_malloc(Arena *arena, size_t size) {
  ...
  //
  // Optimally that wouldn't be necessary because in most cases we should know
  // an approximately good size for our regions. But there are cases where that
  // wouldn't be the case, primarily when we are not the ones allocating stuff,
  // i.e. the end user of this languag.
  assert(
  ...
  if (size <= region->remainder) {
    ...
    log_debug("end alloc mblock(%p) [size=%lld, arena=%p, mregion=%p]", ptr, size,
              arena, region);
  ...
  MemRegion *new_region = (MemRegion *)malloc(sizeof(
                            MemRegion));
  if (!new_region) {
    log_fatal("alloc mregion [arena=%p]", arena);
    return NULL;
  ...
  new_region->data = (char *)calloc(arena->region_cap,
                                    sizeof(char));
  if (!new_region->data) {
  ...
  log_debug("alloc data(%p) [arena=%p, region=%p, size=%lld]", new_region->data,
            arena, new_region, size);

  new_region->next = arena->top;
  arena->top = new_region;
  log_debug("alloc mregion(%p) [arena=%p, cap=%ll, used=%d]", new_region, arena,
            arena->region_cap,
            size);

  // Just to conform to the default behaviour (not creating new mregion).
  char *ptr = new_region->data;
  log_debug("end alloc mblock(%p) [arena=%p, size=%lld, mregion=%p]", ptr, size,
            arena, region);
