...
static LF_TokenType _lf_get_token_type_from_big_lexeme(LF_String *str) {
  ...
  LF_StringComparison comp = {.type = LF_STR_COMPARISON_STR_CHAR, .left = str};
...
static void _lf_flush_lexeme(LF_List *tokens, LF_String *lexeme) {
  ...
  if (lf_get_string_size(lexeme) <= 1) {
    ...
    LF_TokenType token_type =
      _lf_get_token_type_from_soft_boundary_lexeme(*fst_char);
...
void lf_lex(LF_List *tokens,
  ...
  while (cur_char) {
    if (*cur_char == '"' && !is_reading_str) {
      LF_ArenaSlice *str_lexeme_slice =
        (LF_ArenaSlice *)lf_alloc_list_elem(lexeme_slices);
      ...
      str_lexeme_slice->tail_offset =
        src_iter.block_offset + (sizeof(char) * 2);
      ...
      // Skip " and first char in string.
      lf_iterate_string(&src_iter);
      ...
      lf_grow_arena_slice_tail(cur_lexeme->slice,
                               cur_lexeme->arena,
                               sizeof(char));
    ...
    LF_TokenType hard_boundary_token_type =
    ...
    if (has_new_lexeme) {
      LF_ArenaSlice *cur_lexeme_slice =
        (LF_ArenaSlice *)lf_alloc_list_elem(lexeme_slices);
